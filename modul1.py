...
def engineToko(after, dataUpdate):
    ...
    if (len(hasilQwery) > 0):
        ...
        for data in hasilQwery:
            ...
            print(integrasi)
            for i in range(3):  # 0 = no_rek; 1 = tgl; 2=total
        ...
        for i in range(len(update)):
            data = update[i].split("#")
            curToko.execute("UPDATE tb_integrasi SET /*%s*/= %s WHERE id_transaksi = %s;", (data[3],data[2], data[1], data[0]))
            connToko.commit()
        print("data berhasil di update")
    else :
...
def engineBank(after, dataUpdate):
    ...
    bankUpdate = """SELECT id_transaksi FROM tb_transaksi
                WHERE id_transaksi NOT IN (SELECT tb_transaksi.`id_transaksi` FROM tb_transaksi
                                            ...
                                            WHERE tb_transaksi.`rekening_tujuan` = tb_integrasi.`rekening_pengirim` AND 
                                                  tb_transaksi.`tgl_transaksi` = tb_integrasi.`tgl_transaksi` AND
                                                  tb_transaksi.`nominal_transaksi` =tb_integrasi.`nominal_transaksi`
                                            GROUP BY id_transaksi);"""
    ...
    if (len(dataUpdate) > 0 and after ==1 ):
        for data in dataUpdate:
            ...
            curBank.execute("""UPDATE tb_transaksi SET rekening_tujuan = %s, tgl_transaksi = %s, nominal_transaksi = %s
                            WHERE id_transaksi = %s""", (hasil[0][1], hasil[0][2], hasil[0][3], hasil[0][0]))
            connBank.commit()
        print("data berhasil di update")
    ...
    if (len(hasilQwery) > 0):
        ...
        for i in range(len(update)):
            data = update[i].split("#")
            curBank.execute("UPDATE tb_integrasi SET /*%s*/= %s WHERE id_transaksi = %s;", (data[3],data[2], data[1], data[0]))
            connBank.commit()
        print("data berhasil di update")
    else:
...
while(1):
    ...
    #      print("ERROR!")
        time.sleep(10) # memberikan time delay selama 5 detik